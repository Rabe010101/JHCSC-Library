document.addEventListener("DOMContentLoaded", () => {
    ...
    const otpIssuedIdInput = document.getElementById("otpIssuedId"); // Changed ID
    ...
    function fetchIssuedBooks() {
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    tableBody.innerHTML = '<tr><td colspan="8" style="text-align:center;">No issued books found.</td></tr>';
                    return;
                }
        ...
        if (e.target.classList.contains("btn-action")) {
            if (confirm("This will send an OTP to the user's email to confirm the return. Proceed?")) {
                ...
                .then(result => {
                    ...
                    if (result.success) {
                        fetchIssuedBooks(); // Re-fetch to update button
                    } else {
    ...
    // --- Initial Load ---
    searchBox.addEventListener('input', fetchIssuedBooks);
    fetchIssuedBooks();
});